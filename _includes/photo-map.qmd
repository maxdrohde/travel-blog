...
photoMap = {
  ...
  function initMap() {
    ...
    let tileLayer = L.tileLayer(isDark() ? DARK_URL : LIGHT_URL, TILE_OPTS).addTo(map);
    ...
    for (const [, g] of grouped) {
      ...
      if (count > 1) {
        const badge = L.divIcon({
          html: `<div style="font-family: 'IBM Plex Mono', monospace; font-size: 0.6rem; color: #fff; background: ${accentColor()}; border-radius: 50%; width: 16px; height: 16px; display: flex; align-items: center; justify-content: center; font-weight: 700;">${count}</div>`,
          iconSize: [16, 16],
          iconAnchor: [8, 8],
    ...
    new MutationObserver(() => {
      ...
      tileLayer = L.tileLayer(dark ? DARK_URL : LIGHT_URL, TILE_OPTS).addTo(map);
      const color = accentColor();
      markers.forEach(m => {
        if (m.setStyle) m.setStyle({ fillColor: color });
        if (m._badgeCount) m.setIcon(L.divIcon({
          html: `<div style="font-family: 'IBM Plex Mono', monospace; font-size: 0.6rem; color: #fff; background: ${color}; border-radius: 50%; width: 16px; height: 16px; display: flex; align-items: center; justify-content: center; font-weight: 700;">${m._badgeCount}</div>`,
