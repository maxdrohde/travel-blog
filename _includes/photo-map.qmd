...
photoMap = {
  ...
  function initMap() {
    ...
    function formatDate(exifDate) {
      ...
      return `${month} ${+d}, ${y} · ${hour}:${min} ${ampm}`;
    }
    ...
    function photoPopupHtml(photos, groupId) {
      const cid = `mapCarousel-${groupId}`;
      const multi = photos.length > 1;
      const indicators = multi ? `<div class="carousel-indicators">${photos.map((_, i) =>
        `<button type="button" data-bs-target="#${cid}" data-bs-slide-to="${i}"${i === 0 ? ' class="active"' : ''}></button>`
      ).join("")}</div>` : "";
      const slides = photos.map((loc, i) =>
        ...
        `<img src="${loc.file}" class="d-block w-100" style="height: 200px; object-fit: cover;" alt="">` +
        `</a>` +
        (loc.date ? `<div class="carousel-caption"><p>${formatDate(loc.date)}</p></div>` : "") +
        `</div>`
      ).join("");
      ...
      return `<div id="${cid}" class="carousel slide" data-bs-ride="false" style="width: 280px; border-radius: 4px; overflow: hidden;">` +
        indicators +
    ...
    for (const [, g] of grouped) {
      ...
      // Initialize carousel + GLightbox on popup links after popup opens
      function onPopupOpen() {
        const carouselEl = document.getElementById(`mapCarousel-${gid}`);
        if (carouselEl && typeof bootstrap !== "undefined") {
          new bootstrap.Carousel(carouselEl, { ride: false, interval: false });
        }
